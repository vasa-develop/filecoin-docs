...
In order to make testing easier, a live devnet (developer network) is now available for use. We call this network the Nerpa Devnet (the nerpa is a species of seal, and one of the smallest true seals). The Nerpa Devnet features 512MiB sectors (in addition to 32GiB and 64GiB sectors) and reduced proofs parameters, so sealing is much faster than in the full Testnet version.
...
3. There should be a few miners on the network that are configured to auto-accept storage deals. If you are having trouble finding a miner to accept your storage deal, please ping the #fil-storage-dev channel on the [Filecoin slack](https://filecoin.io/slack).

To tap the faucet or see dashboard:
